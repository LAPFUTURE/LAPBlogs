...
def editBlog(request):
    if request.POST:
        try:
            currentTime = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            blog = Blogs.find_one(ObjectId(request.POST["blogId"]))
            blog["content"] = request.POST["content"]
            blog["title"] = request.POST["title"]
            blog["insertTime"] = currentTime
            blog["type"] = request.POST["type"]
            condition = {"_id":ObjectId(request.POST["blogId"])}
            Blogs.update(condition,blog) #更新Blogs表
            obj = {"message":"编辑成功","status":1}
        except BaseException as e:
            obj = {"message":"服务器出错","status":-2}
    else:
        obj = {"message":"请求方式不是post!","status":-5}
